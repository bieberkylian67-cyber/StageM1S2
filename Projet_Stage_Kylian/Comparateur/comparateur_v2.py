            ...
            for start, end in rassembler_exons:
                ...
                for i, (s_ens, e_ens) in enumerate(liste_exon_ensembl):
                    ...
                    # CAS MISMATCH (Chevauchement)
                        ...
                        message = "Mismatch"
                        decalage_start = start_augustus - s_ens
                        decalage_end = end_augustus - e_ens
                        trouve = True
                        coches_ensembl[i] = True # On coche !
                        break
                
                # On écrit le résultat pour cet exon d'Augustus
                fichier.write(f"{id_transcript}\t{start_augustus}\t{end_augustus}\t{message}\t{decalage_start}\t{decalage_end}\n")

            #4. DERNIÈRE ÉTAPE : LES DÉLÉTIONS
            # On regarde quels exons Ensembl n'ont jamais été "cochés"
            for i, (s_ens, e_ens) in enumerate(liste_exon_ensembl):
                if coches_ensembl[i] == False:
                    fichier.write(f"{id_transcript}\t{s_ens}\t{e_ens}\tDélétion\t-\t-\n")
...
if __name__ == "__main__":
    ...
    dico_ens = dictionnaire_ensembl(fichier_input_ensembl)

    # 3. On génère le fichier de statistiques/annotations
    annotations(dico_conv, dico_ens, fichier_annotation)

    # 4. (Optionnel) Ton filtre de mauvaises prédictions
    filtre_mauvaise_prediction(dico_conv, dico_ens, dataset_bad_predictions)
