...
def makeAsyncEnsemblSeqRequest(ID_file, type_request):
    ...
    rs = [grequests.post(url, headers=headers, data=json.dumps(i))
          for i in params]
    all_response = list(tqdm(grequests.imap(rs, size=10), total=len(rs), desc="Séquences"))
...
def makeAsyncEnsemblExonmapRequest(ID_file):
    ...
    rs = [grequests.post(url, headers=headers, data=json.dumps(i))
          for i in params]
    all_response = list(tqdm(grequests.imap(rs, size=10), total=len(rs), desc="Exon map"))
