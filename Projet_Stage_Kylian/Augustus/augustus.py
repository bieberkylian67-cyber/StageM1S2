...
with open(fichier_sortie, "w") as f:
    for sequences in SeqIO.parse(fichier_input, "fasta"):
        ...
        commande = ["augustus", f"--species={species}", "/dev/stdin"]
        resultat = subprocess.run(commande, input=sequences.format("fasta"), text=True, capture_output=True)
        f.write(resultat.stdout.replace("unnamed-1", sequences.id))  